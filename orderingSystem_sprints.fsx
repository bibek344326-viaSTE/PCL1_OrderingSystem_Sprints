...
let processPayment payment =
    match payment with
    | ViaCard -> printfn "Processing payment via ViaCard..."
    | CreditCard -> printfn "Processing payment via Credit Card..."
    | MobilePay -> printfn "Processing payment via MobilePay..."
...
type OrderProductMsg =
    | Order of Product * quantity:int * payment:PaymentType
    ...
    let agent =
        MailboxProcessor<OrderProductMsg>.Start(fun inbox ->
            ...
            let rec messageLoop () = async {
                ...
                | Order (product, quantity, payment) ->
                    ...
                    printfn "Please pay DKK %.2f for %d %A(s) using %A. Thank you!"
                        totalPrice quantity product payment
...
agent.Post (Order (Drink { Category = Coffee Latte; Size = Small }, 2, ViaCard))
agent.Post (Order (Food Burger, 1, CreditCard))
agent.Post (Order (Fruit Apple, 3, MobilePay))
